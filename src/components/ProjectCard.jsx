      ...
      <div className="p-5 flex flex-col flex-1">
        ...
        <div className="mt-3 flex flex-wrap gap-1.5">
          {project.technologies.slice(0, 4).map((tech) => (
            <span key={tech} className="px-2 py-0.5 rounded text-[11px] font-mono text-ink-muted bg-white/5">
              {tech}
            </span>
          ))}
        ...
        <div className="mt-5 flex items-center justify-between">
          <div className="flex items-center gap-3">
            {project.github ? (
              ...
              <button
                type="button"
                onClick={() => setNotice('GitHub link isn\u2019t available yet.')}
                aria-label={`${project.title} GitHub repository`}
            ...
            {project.liveDemo ? (
              ...
              <button
                type="button"
                onClick={() => setNotice('Live demo isn\u2019t available yet.')}
                aria-label={`${project.title} live demo`}
                className="text-ink-muted/40 hover:text-ink-muted transition-colors"
              >
                <ExternalLink size={17} />
