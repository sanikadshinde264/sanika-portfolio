    ...
    >
      ...
      <div className="relative w-full sm:max-w-2xl bg-bg-card sm:rounded-2xl border border-white/10 my-0 sm:my-auto">
        ...
        <div className="px-6 sm:px-8 pb-8 -mt-10 sm:-mt-4">
          ...
          <h3 id="project-modal-title" className="mt-1 font-display text-2xl font-extrabold text-ink">
            {project.title}
          </h3>
          ...
          <div className="mt-6 space-y-6">
            <div>
              <h4 className="text-sm font-semibold text-ink uppercase tracking-wide mb-2">Overview</h4>
              <p className="text-sm text-ink-muted leading-relaxed">{project.description}</p>
            </div>

            {project.problem && (
              <div>
                <h4 className="text-sm font-semibold text-ink uppercase tracking-wide mb-2">Problem Statement</h4>
            ...
            {project.solution && (
              <div>
                <h4 className="text-sm font-semibold text-ink uppercase tracking-wide mb-2">Solution</h4>
                <p className="text-sm text-ink-muted leading-relaxed">{project.solution}</p>
              </div>
            )}

            <div>
              <h4 className="text-sm font-semibold text-ink uppercase tracking-wide mb-2">Key Features</h4>
              <ul className="grid sm:grid-cols-2 gap-x-4 gap-y-1.5">
                {project.highlights.map((h) => (
                  <li key={h} className="text-sm text-ink-muted flex gap-2">
                    <span className="text-success mt-1.5 text-[8px]">●</span>
                    <span>{h}</span>
                  </li>
                ))}
              </ul>
            ...
            {project.dataset && (
              <div>
                <h4 className="text-sm font-semibold text-ink uppercase tracking-wide mb-2">Dataset</h4>
                <ul className="space-y-1">
                  {project.dataset.map((d) => (
                    <li key={d} className="text-sm text-ink-muted">{d}</li>
                  ))}
                </ul>
              </div>
            )}
            ...
            {project.method && (
              <div>
                <h4 className="text-sm font-semibold text-ink uppercase tracking-wide mb-2">Methodology</h4>
                <div className="flex flex-wrap gap-2">
                  {project.method.map((m) => (
                    <span key={m} className="px-2.5 py-1 rounded-md text-xs font-mono text-ink-muted bg-white/5">
                      {m}
                    </span>
                  ))}
                </div>
            ...
            <div>
              <h4 className="text-sm font-semibold text-ink uppercase tracking-wide mb-2">Technologies</h4>
              <div className="flex flex-wrap gap-2">
                {project.technologies.map((tech) => (
                  <span key={tech} className="px-2.5 py-1 rounded-md text-xs font-mono text-accent bg-accent/10">
                    {tech}
                  </span>
                ))}
              </div>
            ...
            {project.futureImprovements && (
              <div>
                <h4 className="text-sm font-semibold text-ink uppercase tracking-wide mb-2">Future Improvements</h4>
                <div className="flex flex-wrap gap-2">
                  {project.futureImprovements.map((f) => (
                    <span key={f} className="px-2.5 py-1 rounded-md text-xs font-mono text-ink-muted bg-white/5">
                      {f}
                    </span>
                  ))}
                </div>
          ...
          <div className="mt-8">
            <div className="flex flex-wrap gap-3">
              {project.github ? (
                ...
                <button
                  type="button"
                  onClick={() => setNotice('GitHub link isn\u2019t available for this project yet.')}
                  className="inline-flex items-center gap-2 px-4 py-2.5 rounded-lg border border-white/10 text-ink text-sm font-semibold hover:bg-white/5 transition-colors"
                >
                  <Github size={16} /> View on GitHub
                </button>
              ...
              {project.liveDemo ? (
                ...
                <button
                  type="button"
                  onClick={() => setNotice('Live demo isn\u2019t available for this project yet.')}
                  className="inline-flex items-center gap-2 px-4 py-2.5 rounded-lg bg-accent text-bg text-sm font-semibold hover:bg-accent/90 transition-colors"
                >
                  <ExternalLink size={16} /> Live Demo
                </button>
